...
function createComment(url, request){
  ...
  if (requestComment && requestComment.body && requestComment.articleId && requestComment.username) 
  {

    if (database.articles[requestComment.articleId] && database.users[requestComment.username]){
...
function updateComment(url, request)
{
  ...
  */
    ...
    if ( !oldComment){
      ...
      response.body = {comment: oldComment};
      response.status = 200;
    }
...
function deleteComment(url, request){
  ...
  const response = {};
   ...
   if (!comment){
    ...
    const rangArticle = database.articles[articleId].commentIds.indexOf(id);
    const rangUser = database.users[username].commentIds.indexOf(id);
    
    database.articles[articleId].commentIds.splice(rangArticle,1);
    database.users[username].commentIds.splice(rangUser,1);
